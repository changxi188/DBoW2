...
template <class TDescriptor, class F>
TemplatedDatabase<TDescriptor, F>::TemplatedDatabase(const std::string& filename) : m_voc(NULL)
{
    load(filename);
}
...
template <class TDescriptor, class F>
TemplatedDatabase<TDescriptor, F>::TemplatedDatabase(const char* filename) : m_voc(NULL)
{
    load(filename);
}
...
template <class TDescriptor, class F>
void TemplatedDatabase<TDescriptor, F>::save(cv::FileStorage& fs, const std::string& name) const
{
    // Format YAML:
...
template <class TDescriptor, class F>
void TemplatedDatabase<TDescriptor, F>::load(const cv::FileStorage& fs, const std::string& name)
{
    // load voc first
    ...
    if (m_use_di)
    {
        ...
        for (EntryId eid = 0; eid < fn.size(); ++eid)
        {
            ...
            for (unsigned int i = 0; i < fe.size(); ++i)
            {
                ...
                dit = m_dfile[eid].insert(m_dfile[eid].end(), make_pair(nid, std::vector<unsigned int>()));
